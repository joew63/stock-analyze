    ...
    <div className="space-y-3">
      {news.map((item) => (
        <a
          ...
          className="block rounded-xl border border-neutral-800 bg-neutral-900/60 p-4 transition hover:border-neutral-600"
        >
          <div className="flex items-center justify-between text-xs text-neutral-500">
            <span>{item.source}</span>
            <span>{timeAgo(item.datetime)}</span>
          </div>
          <h4 className="mt-1 text-sm font-medium text-neutral-100">{item.headline}</h4>
          {item.summary && (
